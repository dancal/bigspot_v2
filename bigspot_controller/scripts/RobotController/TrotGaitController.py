...
class TrotGaitController(GaitController):
    def __init__(self, body, legs, stance_time, swing_time, time_step, use_imu):
        ...
        self.state              = State(self.default_height, BehaviorState.TROT)
        self.command            = Command(self.default_height)

        super().__init__(stance_time, swing_time, time_step, contact_phases, self.default_stance)
        #self.max_x_velocity = 0.024 #[m/s] 
        #self.max_y_velocity = 0.015 #[m/s]

        self.max_x_velocity = 0.12 #[m/s] 
        self.max_y_velocity = 0.02 #[m/s]
        self.max_yaw_rate   = 0.6 #[rad/s]

        self.swingController = TrotSwingController(self.stance_ticks, self.swing_ticks, self.time_step, self.phase_length, z_leg_lift, self.default_stance)
        self.stanceController = TrotStanceController(self.phase_length, self.stance_ticks, self.swing_ticks, self.time_step, z_error_constant)
